...
video = cv2.VideoCapture('baby_carlos.mov')

_, old_frame = video.read()
frame_counter = frame_counter + 1
old_gray = cv2.cvtColor(old_frame, cv2.COLOR_BGR2GRAY)
prev_pts = cv2.goodFeaturesToTrack(image=old_gray, maxCorners=MAX_NUM_CORNERS, qualityLevel=0.3, minDistance=7, blockSize=7)
...
while True:
    ...
    nextPts, statuses, _ = cv2.calcOpticalFlowPyrLK(prevImg=old_gray, nextImg=frame_gray, prevPts=prev_pts, 
            nextPts=None,
            winSize=(15,15),
            maxLevel=2,
            criteria=(cv2.TERM_CRITERIA_COUNT | cv2.TERM_CRITERIA_EPS, 10, 0.03))

    good_next_pts = nextPts[statuses==1]
    ...
    for i, (next, old) in enumerate(zip(good_next_pts, good_old_pts)):
        ...
        # Drawing
        cv2.line(mask, (x,y), (r,s), colors[i].tolist(), 2)
        cv2.circle(frame, (x,y), 5, colors[i].tolist(), -1)
        ...
        if distance > DISTANCE_THRESH:
            ...
            print(speed_str)
            cv2.putText(mask_text, speed_str, (x,y), cv2.FONT_HERSHEY_TRIPLEX, 0.5, colors[i].tolist())
